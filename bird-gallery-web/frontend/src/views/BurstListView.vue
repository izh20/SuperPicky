  ...
  <div class="flex gap-6 max-w-7xl mx-auto">
    ...
    <aside class="w-56 flex-shrink-0 space-y-4">
      <div class="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
        ...
        <!-- 鸟种 -->
        <div>
          <label class="text-xs text-gray-500 mb-1 block">鸟种</label>
          <select
        ...
        <!-- 相机 -->
        <div>
          <label class="text-xs text-gray-500 mb-1 block">相机</label>
          <select
        ...
        <!-- 最低评分 -->
        <div>
          <label class="text-xs text-gray-500 mb-1 block">最低评分</label>
          <select
        ...
        <!-- 最少张数 -->
        <div>
          <label class="text-xs text-gray-500 mb-1 block">最少张数</label>
          <input
            type="number"
        ...
        <!-- 置信度阈值 -->
        <div>
          <label class="text-xs text-gray-500 mb-1 block">鸟种置信度 ≥ {{ confidenceMin }}%</label>
          <input
            type="range"
        ...
        <div>
          ...
          <div v-if="burstStore.filters.has_flying" class="mt-1.5">
            <label class="text-xs text-gray-500 mb-1 block">最少飞版张数</label>
            <input
              type="number"
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-bold text-gray-800">
          连拍组
          <span class="text-sm font-normal text-gray-400 ml-2">{{ burstStore.list.length }} 组</span>
        </h1>
        <div class="flex items-center gap-2">
          ...
          <template v-if="selectMode">
            <button
              @click="selectAll"
              class="flex items-center gap-1 px-2.5 py-1.5 text-sm text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50"
            >
              {{ selected.size > 0 ? '取消全选' : `全选(${burstStore.list.length})` }}
            </button>
            <span v-if="selected.size > 0" class="text-sm text-gray-500">已选 {{ selected.size }}</span>
            <button
      ...
      <!-- 检测连拍对话框 -->
      <div v-if="showDetectDialog" class="mb-4 bg-white border border-primary-200 rounded-lg p-4 shadow-sm">
        <h3 class="text-sm font-semibold text-gray-700 mb-3">连拍检测设置</h3>
        <div class="flex items-end gap-4">
          <div>
            <label class="text-xs text-gray-500 mb-1 block">时间阈值（秒）</label>
            <input
              v-model.number="detectThreshold"
              type="number" min="0.05" max="60" step="0.5"
              class="w-24 text-sm border border-gray-200 rounded-lg px-2 py-1.5"
            />
          </div>
          <div>
            <label class="text-xs text-gray-500 mb-1 block">最少帧数</label>
            <input
          ...
          <button
            ...
            class="px-3 py-1.5 text-gray-500 text-sm rounded-lg hover:bg-gray-100"
          >
            取消
          </button>
        </div>
        <p class="text-xs text-gray-400 mt-2">将重新检测所有连拍组。最少帧数默认 15，低于此数量的连拍不会被分组。</p>
      </div>
      ...
      <div v-else class="grid gap-4" style="grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));">
        <div
          ...
          :key="b.id"
          class="bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition-shadow p-3 relative cursor-pointer"
          :class="{ 'ring-2 ring-primary-500': selected.has(b.id) }"
        ...
        >
          ...
          <div
            ...
            :class="selected.has(b.id) ? 'bg-primary-500 border-primary-500' : 'bg-white border-gray-300'"
          >
          ...
          <div class="relative h-28 mb-3">
            ...
            <div class="absolute inset-0 translate-x-2 translate-y-2 bg-gray-200 rounded-lg overflow-hidden">
              <img
            ...
            <div class="absolute top-1 right-1 bg-primary-600 text-white text-xs px-1.5 py-0.5 rounded-full font-bold z-10">
              {{ b.photo_count }}
            </div>
          </div>
